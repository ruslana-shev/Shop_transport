	...
	<div class="container">
		<div class="breadcrumb-product-sledge">
			<ol class="breadcrumb">
			  ...
			  <li class="activ"><a href="bicycles.php">Велосипеды</a></li>
			</ol>
		</div>
		...
		<!-- 		<div class="container"> -->
						<div class="row">
							<?php 
								...
								if ($_GET["under_type"]){
									$check_type_bicycles = implode(',', $_GET["under_type"]);
								}
								...
								if (!empty($check_brand) OR !empty($max_price) OR !empty($check_country)) {
									...
									if (!empty($check_type_bicycles)) $query_type_bicycles = " AND under_type_transport_id IN($check_type_bicycles)";
								...
								$query = "SELECT * FROM table_all_products 
											WHERE visible='1' && type_transport='bicycles' $query_brand $query_price $query_country $query_type_bicycles
											ORDER BY price ASC";
